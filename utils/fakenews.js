...
// Font paths at module level
const FONTS = {
  gugeshashvili: path.resolve(
    __dirname,
    "media",
    "assets",
    "fonts",
    "gugeshashvili_5_mthavruli.ttf",
  ),
  arArchy: path.resolve(
    __dirname,
    "media",
    "assets",
    "fonts",
...
async function mtavari1(image, quote, by) {
  const mtavariOverlay = path.join(
    __dirname,
    "media",
    "assets",
    "mtavari",
    "mtavari1-overlay.png",
  );
  const outputPath = path.join(__dirname, "media", "mtavari1.png");
  const transparentImagePath1 = path.join(
    __dirname,
    "media",
    "outputs",
    "mt1-transparent1.png",
  );
  const transparentImagePath2 = path.join(
    __dirname,
    "media",
    "outputs",
    "mt1-transparent2.png",
  );
  ...
  const textImageQuote = await sharp(transparentImagePath1)
    .composite([
      {
        input: {
          text: {
  ...
  const textImageBy = await sharp(transparentImagePath2)
    .composite([
      {
        input: {
          text: {
  ...
  await sharp(resizedImage)
    .composite([
      ...
      { input: textImageQuote, top: 180, left: 85 },
      { input: textImageBy, top: 785, left: 150 },
    ])
    .toFile(outputPath);
...
async function inforustavi(image, caption) {
  const InfoOverlay = path.join(
    __dirname,
    "media",
    "assets",
    "inforustavi",
    "inforustavi-overlay.png",
  );
